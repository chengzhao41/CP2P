...
for (temp.num in 1:24) {
  # Boxplot Data ---------------------------------
  if (temp.num != 24) {
    load(paste0("~/output_temp/Erlotinib/cpp_var/", label.type, "/erlotinib_cpp_1to100_", label.type, "_var", temp.num, ".RData"))       
  } else {
    load(paste0("~/output_temp/Erlotinib/Used/erlotinib_cpp_1to25_all_", label.type, ".RData"))
  }
...
num_of_patients = seq(1, 24, 1)
stopifnot(dim(cpp.varying_training_matrix2)[1] == length(num_of_patients))
save(cpp.varying_training_matrix2, num_of_patients, label.type, file = paste0("cpp_var_", label.type, ".RData"))
