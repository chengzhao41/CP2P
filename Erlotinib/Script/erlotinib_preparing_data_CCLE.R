...
source('Erlotinib/Script/generate_random_partition.R')
...
ccle_sensitivity_1 <- read.csv("CGP/ccle_sensitivity_1.csv")
dim(ccle_sensitivity_1)

temp.erlotinib_ind <- which(ccle_sensitivity_1$drug == "Erlotinib")
stopifnot(length(temp.erlotinib_ind) > 0)
ccle_sensitivity_1 <- ccle_sensitivity_1[temp.erlotinib_ind, ]
dim(ccle_sensitivity_1)

temp.na_ind <- which(!is.na(ccle_sensitivity_1$slope0.sensitivity.call))
length(temp.na_ind)
temp.response_slope <- ccle_sensitivity_1$slope0.sensitivity.call[temp.na_ind]
length(temp.response_slope)
stopifnot(sum(is.na(temp.response_slope)) == 0)
temp.labels_slope <- temp.response_slope == 1

names(temp.labels_slope) <- ccle_sensitivity_1$cellline[temp.na_ind]
temp.ind <- rownames(data.ge.ccle) %in% names(temp.labels_slope)
temp.slope_ind <- match(rownames(data.ge.ccle)[temp.ind], names(temp.labels_slope))
stopifnot(names(temp.labels_slope)[temp.slope_ind] == rownames(data.ge.ccle)[temp.ind])
stopifnot(sum(is.na(temp.labels_slope)) == 0)

erlotinib.labels$slope <- temp.labels_slope[temp.slope_ind]
erlotinib.labels$slope_ind <- which(rownames(data.ge.ccle) %in% names(temp.labels_slope))
length(erlotinib.labels$slope_ind)

stopifnot(names(erlotinib.labels$slope) == rownames(data.ge.ccle)[erlotinib.labels$slope_ind])

### END
###
temp.drug_ind <- which(druginfo.ccle$drugid == "ERLOTINIB")
stopifnot(length(temp.drug_ind) == 1)

temp.ic50 <- drugpheno.ccle$IC50[, temp.drug_ind]
temp.ic50_ind <- which(!is.na(temp.ic50))
temp.ic50 <- temp.ic50[temp.ic50_ind]
length(temp.ic50)

temp.auc <- drugpheno.ccle$AUC[, temp.drug_ind]
...
erlotinib.labels$IC50_ind <- which(rownames(data.ge.ccle) %in% names(temp.label_IC50))
erlotinib.labels$AUC_ind <- which(rownames(data.ge.ccle) %in% names(temp.label_auc))

stopifnot(names(temp.label_IC50) == rownames(data.ge.ccle)[erlotinib.labels$IC50_ind])
erlotinib.labels$IC50 <- temp.label_IC50
stopifnot(names(temp.label_auc) == rownames(data.ge.ccle)[erlotinib.labels$AUC_ind])
...
# 388 / 446 in with IC50 and slope
...
# 388 / 446 with AUC and slope

###################### 
## get the data for slopes
erlotinib <- list()
erlotinib$ccle_slope <- data.ge.ccle[erlotinib.labels$slope_ind,  ]
erlotinib$ccle_IC50 <- data.ge.ccle[erlotinib.labels$IC50_ind,  ]
erlotinib$ccle_AUC <- data.ge.ccle[erlotinib.labels$AUC_ind,  ]

rm(temp)
temp <- remove_features(input_data = erlotinib$ccle_slope, input_ind = erlotinib.labels$slope_ind,
                        input_label = erlotinib.labels$slope)
erlotinib$ccle_slope <- temp$data
erlotinib.labels$slope_ind <- temp$ind
erlotinib.labels$slope <- temp$label

rm(temp)
temp <- remove_features(input_data = erlotinib$ccle_IC50, input_ind = erlotinib.labels$IC50_ind,
                        input_label = erlotinib.labels$IC50)
erlotinib$ccle_IC50 <- temp$data
erlotinib.labels$IC50_ind <- temp$ind
erlotinib.labels$IC50 <- temp$label

rm(temp)
temp <- remove_features(input_data = erlotinib$ccle_AUC, input_ind = erlotinib.labels$AUC_ind,
                        input_label = erlotinib.labels$AUC)
erlotinib$ccle_AUC <- temp$data
erlotinib.labels$AUC_ind <- temp$ind
erlotinib.labels$AUC <- temp$label

stopifnot(!is.na(mean(erlotinib$ccle_AUC)))
stopifnot(!is.na(mean(erlotinib$ccle_IC50)))
stopifnot(!is.na(mean(erlotinib$ccle_slope)))
...
sum(!is.na(temp.ind))
# 492 /  492 for AUC vs. IC50 

# Using sva to harmonize across different tissue types
show_pca(input_data = erlotinib$ccle_slope, label = erlotinib.labels$slope)
show_pca(input_data = erlotinib$ccle_AUC, label = erlotinib.labels$AUC)
show_pca(input_data = erlotinib$ccle_IC50, label = erlotinib.labels$IC50)

##
temp <- erlotinib
temp.labels <- erlotinib.labels
# get patient data 
load("Erlotinib/WS/clinical_data_and_GEO.RData")
# merge the previous fields
erlotinib <- merge(erlotinib, temp)
erlotinib.labels <- merge(erlotinib.labels, temp.labels)

# add the gene symbols
colnames(erlotinib$ccle_IC50) = annot.ge.ccle$symbol
colnames(erlotinib$ccle_AUC) = annot.ge.ccle$symbol
colnames(erlotinib$ccle_slope) = annot.ge.ccle$symbol

## GEO and CCLE lung only 
temp.ccle_lung_only.IC50 <- which(sampleinfo.ccle$tissue.type[erlotinib.labels$IC50_ind] == "lung")
temp.data <- comGENE(erlotinib$GEO_IC50, scale(erlotinib$ccle_IC50[temp.ccle_lung_only.IC50, ]))
mean(temp.data[[1]])
mean(temp.data[[2]])

temp.include_GEO <- which(!(names(erlotinib.labels$GEO_IC50) %in% names(erlotinib.labels$IC50)))
erlotinib.labels$lung_all.IC50 <- c(erlotinib.labels$GEO_IC50[temp.include_GEO], erlotinib.labels$IC50[temp.ccle_lung_only.IC50], erlotinib.labels$patient)

temp.data[[1]] = temp.data[[1]][temp.include_GEO, ]
erlotinib.labels$lung_all.IC50.source <- c(rep("GEO", dim(temp.data[[1]])[1]), rep("CCLE", dim(temp.data[[2]])[1]), rep("patient", length(erlotinib.labels$patient)))
...
#show_pca(input_data = erlotinib$lung_all.IC50.SVA, label = erlotinib.labels$lung_all.IC50.source)

# slope
temp.ccle_lung_only.slope <- which(sampleinfo.ccle$tissue.type[erlotinib.labels$slope_ind] == "lung")
temp.data <- comGENE(erlotinib$GEO_IC50, scale(erlotinib$ccle_slope[temp.ccle_lung_only.slope, ]))
mean(temp.data[[1]])
mean(temp.data[[2]])

temp.include_GEO <- which(!(names(erlotinib.labels$GEO_IC50) %in% names(erlotinib.labels$slope)))
erlotinib.labels$lung_all.slope <- c(erlotinib.labels$GEO_IC50[temp.include_GEO], erlotinib.labels$slope[temp.ccle_lung_only.slope], erlotinib.labels$patient)
temp.data[[1]] = temp.data[[1]][temp.include_GEO, ]
erlotinib.labels$lung_all.slope.source <- c(rep("GEO", dim(temp.data[[1]])[1]), rep("CCLE", dim(temp.data[[2]])[1]), rep("patient", length(erlotinib.labels$patient)))
...
## all of CCLE + GEO
# IC50
temp.data <- comGENE(erlotinib$GEO_IC50, scale(erlotinib$ccle_IC50))
mean(temp.data[[1]])
mean(temp.data[[2]])

temp.include_GEO <- which(!(names(erlotinib.labels$GEO_IC50) %in% names(erlotinib.labels$IC50)))
temp.data[[1]] = temp.data[[1]][temp.include_GEO, ]
erlotinib.labels$all.source.IC50 <- c(rep("GEO", dim(temp.data[[1]])[1]), rep("CCLE", dim(temp.data[[2]])[1]), rep("patient", length(erlotinib.labels$patient)))
erlotinib.labels$all.IC50 <- c(erlotinib.labels$GEO_IC50[temp.include_GEO], erlotinib.labels$IC50, erlotinib.labels$patient)
...
# slope
temp.data <- comGENE(erlotinib$GEO_IC50, scale(erlotinib$ccle_slope))
mean(temp.data[[1]])
mean(temp.data[[2]])

temp.include_GEO <- which(!(names(erlotinib.labels$GEO_IC50) %in% names(erlotinib.labels$IC50)))
temp.data[[1]] = temp.data[[1]][temp.include_GEO, ]
erlotinib.labels$all.source.slope <- c(rep("GEO", dim(temp.data[[1]])[1]), rep("CCLE", dim(temp.data[[2]])[1]), rep("patient", length(erlotinib.labels$patient)))
erlotinib.labels$all.slope <- c(erlotinib.labels$GEO_IC50[temp.include_GEO], erlotinib.labels$slope, erlotinib.labels$patient)
...
### get l1000 features
stopifnot(colnames(erlotinib$ccle_IC50) == colnames(erlotinib$ccle_slope))

Landmark_Genes_n978 <- read.csv("Common/Landmark_Genes_n978.csv")

feature.l1000 <- list()
feature.l1000$cc <- which(colnames(erlotinib$ccle_slope) %in% Landmark_Genes_n978$Gene.Symbol)
...
getwd()
#setwd("Erlotinib/WS/")
#save(erlotinib, erlotinib.labels, sampleinfo.ccle, partition, feature.l1000, file = "erlotinib_data.RData")

### create partitions for varying number of patients
partition_var <- list()

temp.cp <- foreach (training_amount.cp = seq(from = 30, to = 479, by = 30)) %do% {    
...
# double check
for (temp.ind in 1:length(seq(from = 30, to = 479, by = 30))) {
  ...
  for (temp.ind2 in 1:100) {
    stopifnot(min(table(c(erlotinib.labels$GEO_IC50, erlotinib.labels$slope)[temp.cp[[temp.ind]]$slope[[temp.ind2]]$training_index.single])) >= 5)
    stopifnot(min(table(c(erlotinib.labels$GEO_IC50, erlotinib.labels$slope)[temp.cp[[temp.ind]]$slope[[temp.ind2]]$test_index])) >= 5)
    
    # check for out of range
    stopifnot(min(temp.cp[[temp.ind]]$slope[[temp.ind2]]$test_index) > length(c(erlotinib.labels$GEO_IC50, erlotinib.labels$slope)))
    stopifnot(max(temp.cp[[temp.ind]]$slope[[temp.ind2]]$test_index) <= length(c(erlotinib.labels$GEO_IC50, erlotinib.labels$slope)) + length(erlotinib.labels$patient))
    stopifnot(length(intersect(temp.cp[[temp.ind]]$slope[[temp.ind2]]$test_index, temp.cp[[temp.ind]]$slope[[temp.ind2]]$training_index.single)) == 0)
  }
}
...
temp.cp <- foreach (training_amount.cp = seq(from = 10, to = 126, by = 10)) %do% {    
...
# double check
for (temp.ind in 1:length(seq(from = 10, to = 126, by = 10))) {
  stopifnot(length(temp.cp[[temp.ind]]$slope[[100]]$test_index) == length(erlotinib.labels$patient))
...
temp.cpp <- foreach (cell_line_training_amount = seq(from = 30, to = 479, by = 30)) %do% {  
...
#save(erlotinib, erlotinib.labels, sampleinfo.ccle, partition, feature.l1000, partition_var, file = "Erlotinib/WS/erlotinib_data_CCLE.RData")
