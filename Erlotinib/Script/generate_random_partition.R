...
generate_random_partition.cp_var <- function(input_labels_cell_lines, input_labels_patient, training_amount) {
  ...
  temp.true <- which(input_labels_cell_lines)
  temp.cell_lines <- 1:length(input_labels_cell_lines)
  temp.patients <- length(input_labels_cell_lines) + 1:length(input_labels_patient)    
  ...
  cp.partition <- foreach (temp.run_ind = 1:100, .errorhandling="stop") %dopar% {        
    ...
    temp.training_index.single <- sample(temp.true, 5)    
    temp.training_index.single <- c(temp.training_index.single, sample(temp.cell_lines[-temp.training_index.single], training_amount - 5))
    stopifnot(length(unique(temp.training_index.single)) == length(temp.training_index.single))
    ...
    temp.training_index.single <- sample(temp.training_index.single)
