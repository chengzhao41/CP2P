...
for (i in 1:length(input.response_labels)) {
  ...
  input.WS_path <- paste0("Erlotinib/computed_WS/gdsc/var p/erlotinib_gdsc_cp2p_", input.response_label, "_1to100_parInd")
  input.WS_paths <- rep(input.WS_path, 11)
  input.WS_paths[12] <- paste0("Erlotinib/computed_WS/gdsc/var p/erlotinib_gdsc_cp2p_", input.response_label, "_1to25_parInd")
  input.output_file_name_plot <- paste0("erl_gdsc_cp2p_", input.response_label, ".png")
  input.output_file_name_WS <- paste0("erl_gdsc_cp2p_", input.response_label, ".RData")
  # create WS for plot ------------------------------------------------------
  create_plot_WS(input.seq, input.num_partitions, input.x_axis, 
                 input.WS_paths, input.output_file_name_WS, output_dir, input.type_measure = "auc")
  # create plot -------------------------------------------------------------
...
input.x_axis <- seq(from = 70, to = 331, by = 20)
