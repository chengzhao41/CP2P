...
for (temp.num in 1:15) {
  # Boxplot Data ---------------------------------
  load(paste0("/Users/chengzhao/Dropbox/output_WS/Erlotinib/erlotinib_cpp_1to100_", label.type, "_var", temp.num, ".RData"))
  ...
  cpp.test_auc <- foreach (temp.ind = 1:21, .combine=rbind) %do% {
    if (label.type == "slope" || label.type == "slope_p20") {
      temp.predict <- prediction(cpp.predictions[, temp.ind], erlotinib.labels$all.slope[cpp.test_index])
    } else if (label.type == "auc" || label.type == "auc_p20") {
      temp.predict <- prediction(cpp.predictions[, temp.ind], erlotinib.labels$all.IC50[cpp.test_index])
...
num_of_cell_lines = seq(20, 300, 20)
stopifnot(dim(cpp.varying_training_matrix2)[1] == length(num_of_cell_lines))
save(cpp.varying_training_matrix2, num_of_cell_lines, label.type, file = paste0("Erlotinib/output_WS/cpp_var_", label.type, ".RData"))
...
png(filename = paste0("Erlotinib/plots/erl_cpp_var_", label.type, ".png"), width = 800, height = 800)
...
for (temp.ind in 1:21) {
  lines(num_of_cell_lines, cpp.varying_training_matrix2[, temp.ind], col = cl[(temp.ind - 1) %% 7 + 1], type = 'b', pch=temp.points[temp.ind])
}
