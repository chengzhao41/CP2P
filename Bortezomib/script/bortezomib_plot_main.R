...
input.WS_path <- "Bortezomib/computed_WS/Var P/bortezomib_p2p_1to100_parInd"
...
for (i in 1:length(input.response_labels)) {
  ...
  input.WS_path <- paste0("Bortezomib/computed_WS/Var P/bortezomib_cp2p_", input.response_label, "_1to100_parInd")
...
input.num_partitions <- rep(list(1:100), length(input.seq))
input.x_axis <- seq(from = 60, to = 300, by = 20)
stopifnot(length(input.seq) == length(input.x_axis))
...
for (i in 1:length(input.response_labels)) {
  input.response_label <- input.response_labels[i]
  input.WS_path <- paste0("Bortezomib/computed_WS/Var C 50/bortezomib_", input.response_label, "_1to100_parInd")
...
for (i in 1:length(input.response_labels)) {
  ...
  input.WS_path <- paste0("Bortezomib/computed_WS/Var C 100/bortezomib_", input.response_label, "_1to100_parInd")
  input.output_file_name_plot <- paste0("bor_", input.response_label, ".png")
  input.output_file_name_WS <- paste0("bor_", input.response_label, ".RData")
  # create WS for plot ------------------------------------------------------
  create_plot_WS(input.seq, input.num_partitions, input.x_axis, 
...
create_plot_best(input.x_axis = c2p.x_axis, 
                 ...
                 input.color_manual,
                 input.shape_manual,
                 input.constant_best_ind = which(p2p.x_axis == 100))
