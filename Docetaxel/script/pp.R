...
if (length(args) == 3) {
  ...
  print(paste0("docetaxel_pp_", PARTITION_BEGIN, "to", PARTITION_END, "_var", training_var_amount, ".RData"))
  if (training_var_amount <= 8) {
    INPUT_NFOLDS = training_var_amount
  }
} else {
  stop("args length != 3")
}
...
if (exists("training_var_amount")) {
  save.image(paste0("docetaxel_pp_", PARTITION_BEGIN, "to", PARTITION_END, "_var", training_var_amount, ".RData"))  
} else {
  save.image(paste0("docetaxel_pp_", PARTITION_BEGIN, "to", PARTITION_END, "_", args[3], ".RData"))  
}
