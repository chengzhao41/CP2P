...
label.type <- "auc_breast"

for (temp.num in 14:23) {
  # Boxplot Data ---------------------------------
  if (temp.num != 23) {
    load(paste0("~/output_temp/Docetaxel/cpp_var/breast_only/docetaxel_cpp_1to100_", label.type, "_var", temp.num, ".RData"))       
  } else {
  ...
  cpp.test_auc <- foreach (temp.ind = 1:21, .combine=rbind) %do% {
    if (label.type == "slope_breast") {
      temp.predict <- prediction(cpp.predictions[, temp.ind], docetaxel.labels$slope_breast[cpp.test_index])
    } else if (label.type == "auc_breast") {
      temp.predict <- prediction(cpp.predictions[, temp.ind], docetaxel.labels$AUC_breast[cpp.test_index])
    } else if (label.type == "ic50_breast") {
      temp.predict <- prediction(cpp.predictions[, temp.ind], docetaxel.labels$IC50_breast[cpp.test_index])
    } else {
      stop("label wrong")
    }
