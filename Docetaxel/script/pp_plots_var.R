...
for (temp.num in 13:22) {
  
  if (temp.num != 22) {
    load(paste0("~/output_temp/Docetaxel/pp_var/docetaxel_pp_1to100_var", temp.num, ".RData"))   
  } else {
    load("~/Dropbox/Cell Line Drug Response Prediction Project/Ensemble Of Similarity Networks/Cheng/Archive/Docetaxel/pp.RData")
  }
  ...
  pp.test_auc <- foreach (temp.ind = 1:21, .combine=rbind) %do% {
    temp.predict <- prediction(pp.predictions[, temp.ind], docetaxel.labels$patient[pp.test_index])
...
png(filename = paste0("doc_pp_var.png"), width = 800, height = 800)
...
plot(0, 0, xlim = range(num_of_patients), ylim = c(0.4, 1.0), type = "n", 
     ylab = "",
