generate_random_partition <- function(input_labels_cell_lines, input_labels_patient, test_amount) {
  ...
  temp.combined_labels <- c(input_labels_cell_lines, input_labels_patient)
  partition$patients = temp.patients
  partition$cell_lines = temp.cell_lines

  ## cpp
  cpp.partition <- foreach (temp.run_ind = 1:100, .errorhandling="stop") %do% {        
    
    temp.test_index <- sample(temp.patients, test_amount$cpp)
    temp.training_index.single <- c(temp.cell_lines, setdiff(temp.patients, temp.test_index))
    
    while ((length(table(temp.combined_labels[temp.test_index])) != 2) || (length(table(temp.combined_labels[temp.training_index.single])) != 2)) {
      temp.test_index <- sample(temp.patients, test_amount$cpp)
      temp.training_index.single <- c(temp.cell_lines, setdiff(temp.patients, temp.test_index))
    }
  ...
  ## cp
  cp.partition <- foreach (temp.run_ind = 1:100, .errorhandling="stop") %do% {    
    
    temp.test_index <- cpp.partition[[temp.run_ind]]$test_index  
  ...
  for (temp.run_ind in 1:99) {
    stopifnot(sum(cp.partition[[temp.run_ind]]$test_index == cp.partition[[temp.run_ind + 1]]$test_index) != length(cp.partition[[temp.run_ind + 1]]$test_index))
  ...
  pp.partition <- foreach (temp.run_ind = 1:100, .errorhandling="stop") %do% {    
    ...
    while ((length(table(input_labels_patient[temp.test_index])) != 2) || (length(table(input_labels_patient[temp.training_index.single])) != 2)) {
      temp.test_index <- cpp.partition[[temp.run_ind]]$test_index - length(input_labels_cell_lines) 
      temp.training_index.single <- setdiff(temp.patients2, temp.test_index)      
    }
  ...
  ## cc
  cc.partition <- foreach (temp.run_ind = 1:length(temp.cell_lines), .errorhandling="stop") %do% {    
    
    temp.test_index <- temp.cell_lines[temp.run_ind]
  ...
  for (temp.run_ind in 1:(length(input_labels_cell_lines) - 1)) {
...
generate_random_partition.cp_var <- function(input_labels_cell_lines, input_labels_patient, training_amount) {
  ...
  cp.partition <- foreach (temp.run_ind = 1:100, .errorhandling="stop") %dopar% {        
    ...
    while (length(table(input_combined_labels[temp.training_index.single])) != 2 || min(table(input_combined_labels[temp.training_index.single])) < 5) {
      temp.test_index <- temp.patients
      temp.training_index.single <- sample(temp.cell_lines, training_amount)
    }
...
generate_random_partition.cpp_var <- function(input_labels_cell_lines, input_labels_patient, training_amount) {
  ...
  cpp.partition <- foreach (temp.run_ind = 1:100, .errorhandling="stop") %dopar% {        
    ...
    while ((length(table(input_combined_labels[temp.training_index.single])) != 2) || (length(table(input_combined_labels[temp.test_index])) != 2)  || min(table(input_combined_labels[temp.training_index.single])) < 5) {
      temp.patients_in_training <- sample(temp.patients, training_amount)
      temp.training_index.single <- c(temp.cell_lines, temp.patients_in_training)
      temp.test_index <- setdiff(temp.patients, temp.patients_in_training)      
    }
...
generate_random_partition.pp_var <- function(input_labels_patient, training_amount) {
  ...
  pp.partition <- foreach (temp.run_ind = 1:100, .errorhandling="stop") %dopar% {        
    ...
    while ((length(table(input_labels_patient[temp.training_index.single])) != 2) || (length(table(input_labels_patient[temp.test_index])) != 2)  || min(table(input_labels_patient[temp.training_index.single])) < 5) {
      temp.training_index.single <- sample(temp.patients, training_amount)    
      temp.test_index <- setdiff(temp.patients, temp.training_index.single)
    }
...
# generates random partitions with number of patient and cell line in training set specified
...
generate_random_partition.cpp_var3 <- function(input_labels_cell_lines, input_labels_patient, 
                                               cell_line_training_amount, patient_training_amount,
                                               cell_line_order) {
